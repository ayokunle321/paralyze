...
struct PragmaInsertionPoint
{
  ...
  std::string suggested_pragma; // pragma text to insert

  PragmaInsertionPoint(clang::SourceLocation loc, unsigned line, unsigned col,
                       const std::string& type, bool nested, unsigned depth)
      : location(loc), line_number(line), column_number(col), loop_type(type), is_nested(nested),
        nesting_depth(depth)
  {
  }
};
...
public:
  ...
  const std::vector<PragmaInsertionPoint>& getInsertionPoints() const { return insertion_points_; }
