...
struct LoopInfo
{
  ...
  LoopInfo(clang::Stmt* s, clang::SourceLocation loc, unsigned line, const std::string& type)
  ...
  void addChildLoop(size_t child_index) { child_loop_indices.push_back(child_index); }
  ...
  {
    for (bool safe : function_call_safety)
      if (!safe)
        return true;
    return false;
  }
  ...
  bool hasUnsafeCallsRecursive(const std::vector<LoopInfo>& all_loops) const
  {
    ...
    {
      if (child_idx < all_loops.size() && all_loops[child_idx].hasUnsafeCallsRecursive(all_loops))
      {
        return true;
      }
    }
